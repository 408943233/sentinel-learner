...
paddle_ocr = PaddleOCR(
    ...
    rec_batch_num=1024,
    det_db_thresh=0.1,  # 进一步降低检测阈值，检测更多文本框
    det_db_box_thresh=0.3,  # 进一步降低框阈值，减少文本框合并
    drop_score=0.2,  # 降低分数阈值，保留更多结果
    det_db_unclip_ratio=1.5,  # 减小扩展比例，避免相邻框合并
    max_text_length=25,  # 限制最大文本长度，强制拆分长文本
...
def split_text_by_patterns(text, coord):
    """
    根据多种模式拆分合并的文本
...
def postprocess_ocr_result(text_list, coord_list, image_width):
    """
    后处理OCR结果，根据语义和坐标拆分合并的文本
    例如：将 "8.09成交量" 拆分为 "8.09" 和 "成交量"
    """
    if not text_list or not coord_list:
        return text_list, coord_list
    
    new_text_list = []
    new_coord_list = []
    
    for text, coord in zip(text_list, coord_list):
        parts = split_text_by_patterns(text, coord)
        for part_text, part_coord in parts:
            new_text_list.append(part_text)
            new_coord_list.append(part_coord)
    
    return new_text_list, new_coord_list
...
def check_ocr_box(image_source: Union[str, Image.Image], display_img = True, output_bb_format='xywh', goal_filtering=None, easyocr_args=None, use_paddleocr=False):
    ...
    if use_paddleocr:
        ...
        # 后处理：拆分合并的文本
        text, coord = postprocess_ocr_result(text, coord, w)
    else:  # EasyOCR
        if easyocr_args is None:
            easyocr_args = {}
